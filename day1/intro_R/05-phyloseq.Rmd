...
Using the `phylo_obj` object, subset to a specific Phylum that you are interested in. Build an ordination plot based on the subsetted `phyloseq` object you've created using Bray-Curtis dissimilarities and nMDS (as in the above section) and color the points by the location where they were collected.

*Hint*: subset to a specific Phylum with `subset_taxa()`. 

...
Exercise: What is the count of "Bacteria;Acidobacteria;Holophagae;Holophagales;Holophagaceae;Geothrix" for the first observation listed in the `covariates` data frame collected in "Summer"? 

One way to answer this question is by looking into your data.

```{r}
# start with your covariates dataset 
View(covariates)
# note that the sample JPA_May is the first observation in our data from the summer
# now lets look at the abundances
head(row.names(abundances))
# we are interested in the 5th row
abundances$JPA_May[5]
# or, we could use our square bracket notation in another way 
abundances["Bacteria;Acidobacteria;Holophagae;Holophagales;Holophagaceae;Geothrix", "JPA_May"]
```

The solution above works totally fine! However, if you want to do this without looking into your data there are several ways. For one, we'll use the function `which()`, which tells us which indices of a vector are true. 

```{r}
# an example of the function which 
which(c(TRUE, FALSE, TRUE, TRUE))
# using which to answer our question 
# find all observations for which season is "Summer" and grab the first one
index <- which(covariates$Season == "Summer")[1]
abundances["Bacteria;Acidobacteria;Holophagae;Holophagales;Holophagaceae;Geothrix", index]
```

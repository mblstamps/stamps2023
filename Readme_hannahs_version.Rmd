...
getwd()
utils::unzip("files.zip")
```
5. Before moving on, check to make sure you have 7 ".R" files and 2 ".txt" files:
``` {r}
list.files()
```
...
``` {r}
covariates <- read.csv("files/FWS_covariates.txt", sep = "\t")
abundances <- read.csv("files/FWS_OTUs.txt", sep = "\t", row.names = 1, header = T)
```

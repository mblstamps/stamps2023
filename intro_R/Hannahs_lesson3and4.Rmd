...
abundances_long <-abundances_long %>%
  mutate(phylum = str_split(taxonomy, ";")[[1]][3]) %>% 
  mutate(class  = str_split(taxonomy, ";")[[1]][4]) %>% 
  mutate(order  = str_split(taxonomy, ";")[[1]][5]) %>% 
  mutate(family   = str_split(taxonomy, ";")[[1]][6])
 
unique(abundances_long$fine) 
```

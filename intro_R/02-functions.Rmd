...
Each function has its own arguments which can be found in its documentation. Arguments have expected orders (`head()` expects to receive first the object to take the first values from, then the number of objects). These orders can be overridden by using argument names. Some arguments are required (such as the object for `head()`) and some arguments are optional (such as `n` for `head()`). 
...
```{r, eval = FALSE}
compute_rel_abund <- function(___) { # add a required argument named "index" 
  
  index_abund <- abundances[__,__] # use the argument index to choose the         appropriate column of abundances 
  
  index_tot <- _____(index_abund) # use the correct function to get the total counts for the column of abundances 
  
  index_rel <- index_abund / index_tot
  
  ___ # send the correct value back to the user
}
```

...
```{r}
abund_matrix <- as.matrix(abundances %>% select(!taxonomy))
#abund_matrix <- t(abund_matrix)
distances <- vegdist(abund_matrix, method = "euclidean")
pcoa_plotting_data <- cmdscale(distances, eig= TRUE, k=2)
pcoa_point_data <- as.data.frame(pcoa_plotting_data$points)
pcoa_point_data$taxonomy <- NA
pcoa_point_data$taxonomy <- abundances$taxonomy

pcoa_w_metadata <- inner_join(pcoa_point_data, abundances, by = "taxonomy")
pcoa_w_metadata

#Plot the results with ggplot2
ggplot(pcoa_w_metadata, aes(x=V1, y=V2, label = taxonomy, color = )) + geom_point(size = 3, alpha = .5) +
  # geom_text_repel() +
  xlab("PCoA 1") +
  ylab("PCoA 2") +
  theme_classic()
